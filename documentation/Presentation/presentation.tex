...
\frame{
	...
	\begin{figure}[!htb]
		...
		% text
		\draw (1,1.25) node[draw=none] {Tape};
		\draw (5.25,1.8) node[draw=none] {Schreib- und Lesekopf};
		% triangles
...
\frame{
	...
	\begin{center}
		2-Bit-Kodierung der Symbole auf dem Hardwareband
	\end{center}
  	\begin{figure}[!htb]
		\begin{center}
		\begin{tikzpicture}
...
\frame{
	\frametitle{Erläuterung der Hardware / Bauanleitung}
	\begin{center}
		\includegraphics[scale=0.2]{graphics/hardware.png}
	\end{center}
}
...
\frame{
	\frametitle{Live demo: Double copy}
        \textbf{Aufgabe:} Für jede 0 auf Band 1 zwei 0en auf Band 2 schreiben
\bigskip
...
\bigskip
	...
	\begin{minipage}[t]{5cm}
		\begin{center}
		Kodierung der Symbole auf dem Hardwareband
		\end{center}
  		\begin{figure}[!htb]
		\begin{center}
		...
		\draw[thick,scale=0.5,fill=tuRed] (1,3) rectangle +(1,1);
		\draw[thick,scale=0.5,fill=tuRed] (3,3) rectangle +(1,1);
		\draw (1.25,0.25) node[draw=none,scale=0.6] {\#};

		\draw[thick,scale=0.5] (6,1) rectangle +(1,1);
		\draw[thick,scale=0.5,fill=tuRed] (8,1) rectangle +(1,1);
		\draw[thick,scale=0.5,fill=tuRed] (6,3) rectangle +(1,1);
		\draw[thick,scale=0.5] (8,3) rectangle +(1,1);
		\draw (3.75,0.25) node[draw=none,scale=0.6] {0};

		\draw[thick,scale=0.5,fill=tuRed] (11,1) rectangle +(1,1);
		\draw[thick,scale=0.5] (13,1) rectangle +(1,1);
		\draw[thick,scale=0.5] (11,3) rectangle +(1,1);
		\draw[thick,scale=0.5,fill=tuRed] (13,3) rectangle +(1,1);
		\draw (6.25,0.25) node[draw=none,scale=0.6] {1};

		\draw[thick,scale=0.5,fill=tuRed] (16,1) rectangle +(1,1);
		\draw[thick,scale=0.5,fill=tuRed] (18,1) rectangle +(1,1);
		\draw[thick,scale=0.5] (16,3) rectangle +(1,1);
		\draw[thick,scale=0.5] (18,3) rectangle +(1,1);
		\draw (8.75,0.25) node[draw=none,scale=0.6] {2};
		\end{tikzpicture}
		\end{center}
		\end{figure}
	\end{minipage}		
...
\frame{
	\frametitle{Zusatzfeatures}
        \begin{itemize}
          ...
          \item Schrittweise Simulation
          \item Live-Highlighting der Zustände und Übergänge während
            der Simulation
          \item Simulation ohne Verzögerung
          \item Anzeige der voraussichtlichen Simulationsdauer
          \item Undo \& Redo
          \item Brainfuck-Editor und -Interpreter
...
\frame{
	\frametitle{Binary Multiplication}
        \textbf{Aufgabe:} Multiplikation zweier Binärzahlen 
\bigskip
